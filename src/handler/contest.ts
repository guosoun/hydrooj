            ...
            ...this.user.hasPerm(PERM.PERM_VIEW_HIDDEN_CONTEST) && !group
                ...
                : {
                    $or: [
                        ...
                        {
                            assign: { $in: groups },
                            beginAt: { $gte: this.user.beginAt, $lte: this.user.endAt },
                        },
                        {
                            assign: { $size: 0 },
                            beginAt: { $gte: this.user.beginAt, $lte: this.user.endAt },
...
export class ContestDetailBaseHandler extends Handler {
    ...
    async __prepare(domainId: string, tid: ObjectId) {
        ...
        if (!this.user.own(this.tdoc) && !this.user.hasPerm(PERM.PERM_VIEW_HIDDEN_CONTEST)) {
            if (this.tdoc.beginAt <= this.user.beginAt || this.tdoc.beginAt >= this.user.endAt) {
                throw new NotAssignedError('contest', tid);
            }
        }
...
export class ContestDetailHandler extends Handler {
    ...
    @param('tid', Types.ObjectId)
    async postImportProblemsToUserLibrary(domainId: string, tid: ObjectId) {
        if (!contest.isDone(this.tdoc)) throw new ContestNotEndedError(tid);
        const duduc = await domain.getDomainUser(domainId, this.user._udoc);
        duduc.pids ||= [];
        duduc.pids = [...new Set([...duduc.pids, ...this.tdoc.pids])];
        await domain.setUserInDomain(domainId, this.user._id, duduc);
        this.back();
    }
}
