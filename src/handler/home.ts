...
class HomeSecurityHandler extends Handler {
    ...
    @requireSudo
    @param('beginAt', Types.Date)
    @param('endAt', Types.Date)
    async postChangeActivationTime(domainId: string, beginAt: string, endAt: string) {
        if (!this.session.sudoUid) throw new AccessDeniedError();
        await user.setById(this.user._id, { beginAt: new Date(beginAt), endAt: new Date(endAt) });
        this.back();
    }
