...
export class ProblemModel {
    ...
    static async canViewBy(pdoc: ProblemDoc, udoc: User) {
        ...
        if (udoc.hasPerm(PERM.PERM_VIEW_PROBLEM)) return true;
        if (!udoc.hasPerm(PERM.PERM_VIEW_PROBLEM_IN_CONTEST)) return true;
        const dudoc = await DomainModel.getDomainUser(pdoc.domainId, udoc);
        if (dudoc.pids.includes(pdoc.docId)) return true;
        return false;
    }
