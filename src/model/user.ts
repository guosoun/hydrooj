...
import {
    LoginError, UserAlreadyExistError, UserExpiredError, UserNotFoundError,
} from '../error';
import {
...
export class User {
    ...
    constructor(udoc: Udoc, dudoc, scope = PERM.PERM_ALL) {
        ...
        this.beginAt = udoc.beginAt;
        this.endAt = udoc.endAt;
    ...
    checkExpiration() {
        if (this.endAt && moment(this.endAt).add(system.get('session.grace_period'), 'day') < moment()) {
            throw new UserExpiredError();
        }
    }
